-- ================================================
-- EverMed Development Database Setup (Fixed for Latest Supabase)
...
-- 6. CREATE DEMO USER ACCOUNT (SIMPLIFIED VERSION)
-- ================================================
-- For latest Supabase, we'll create the user and let Supabase handle identities
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = demo_user_id) THEN
        ...
        INSERT INTO auth.users (
            ...
            'authenticated',
            'demo@evermed.ai',
            crypt('123456', gen_salt('bf')), -- Using crypt function
            NOW(),
            NOW(),
            NOW(),
            jsonb_build_object(
        ...
        INSERT INTO auth.identities (
            provider_id,  -- This is the missing field!
            user_id,
            identity_data,
            provider,
            last_sign_in_at,
            ...
            'demo@evermed.ai',  -- provider_id is usually the email for email provider
            demo_user_id,
            jsonb_build_object(
