...
interface FoodEntry {
  ...
  analysisStatus: 'pending' | 'completed' | 'failed'
  ingredients: Ingredient[] // Legacy flat list
  dishes?: Dish[] // Multi-dish structure
  manualIngredients?: Ingredient[] // Ingredients not linked to photos
  totalCalories: number
  totalCarbsG: number
  totalProteinG: number
  totalFatG: number
...
export default function FoodEntryPage({ params }: FoodEntryPageProps) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      router.push('/camera?deleted=true')
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        ...
        {/* Main Card - Food Photos & Header */}
        <div id="meal-summary-card" className="bg-white rounded-2xl shadow-md overflow-hidden">
          {/* Multi-Dish Photo Grid */}
          {foodEntry.dishes && foodEntry.dishes.length > 0 ? (
            <div className={`grid ${foodEntry.dishes.length === 1 ? 'grid-cols-1' : foodEntry.dishes.length === 2 ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-3'} gap-0.5 bg-gray-200`}>
              {foodEntry.dishes.map((dish) => (
                <div
                  key={dish.photoId}
                ...
                >
                  <Image
                    src={dish.photoUrl}
                    alt={`Dish ${dish.dishNumber}`}
                    fill
                    className="object-cover transition-opacity hover:opacity-90"
                    priority={dish.dishNumber === 1}
                    unoptimized
                  />
                  {/* Dish Number Badge */}
                  <div className="absolute top-2 left-2 bg-gray-900 bg-opacity-75 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-semibold">
                    {dish.dishNumber}
                  </div>
        ...
        {/* Ingredients List - Multi-Dish or Legacy */}
        <div className="bg-white rounded-2xl shadow-md p-6 space-y-6">
          ...
          {foodEntry.dishes && foodEntry.dishes.length > 0 ? (
            <div className="space-y-6">
              {foodEntry.dishes.map((dish) => (
                <div key={dish.photoId} className="space-y-4">
                  {/* Dish Header */}
                  <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                    ...
                    <div className="text-right">
                      {dish.analysisStatus === 'pending' && (
                        <span className="text-sm text-amber-600 flex items-center gap-1">
                          <span>⏱️</span>
                          <span>Analyzing...</span>
                        </span>
                      )}
                      {dish.analysisStatus === 'failed' && (
                        <span className="text-sm text-red-600 flex items-center gap-1">
                          <span>✕</span>
                          <span>Failed</span>
                        </span>
                      )}
                      {dish.analysisStatus === 'completed' && (
                        <div className="text-sm">
                          <span className="font-medium text-gray-900">{Math.round(dish.totalCalories)} kcal</span>
                          <span className="text-gray-600 ml-2">• {Math.round(dish.totalCarbsG)}g carbs</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Dish Ingredients */}
                  {dish.analysisStatus === 'completed' && dish.ingredients.length > 0 && (
                  ...
                  {dish.analysisStatus === 'pending' && (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 ml-11">
                      <p className="text-sm text-amber-800 flex items-center gap-2">
                        <span>⏱️</span>
                        <span>Analysis pending for this dish - check back soon!</span>
                      </p>
                    </div>
                  )}

                  {dish.analysisStatus === 'failed' && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-4 ml-11">
                      <p className="text-sm text-red-800 flex items-center gap-2">
                        <span>✕</span>
                        <span>Analysis failed for this dish.</span>
                      </p>
                    </div>
                  )}
                  ...
                  {dish.analysisStatus === 'completed' && dish.ingredients.length === 0 && (
                    <div className="bg-gray-50 rounded-lg p-4 ml-11">
                      <p className="text-sm text-gray-600">
                        No ingredients detected in this dish.
                      </p>
                    </div>
                  )}
                </div>
              ...
              {/* Manual Ingredients (not linked to photos) */}
              {foodEntry.manualIngredients && foodEntry.manualIngredients.length > 0 && (
                <div className="space-y-4 border-t border-gray-200 pt-6">
                  <h3 className="text-base font-semibold text-gray-900">
                    Additional Ingredients
                  </h3>
                  <div className="space-y-3">
                    {foodEntry.manualIngredients.map((ingredient, index) => (
                      <div
                        key={index}
                        className="border border-gray-200 rounded-lg p-4 hover:border-gray-300 transition-all"
                      >
