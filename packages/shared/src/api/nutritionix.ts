 ...
 * Note: Mock imports are environment-specific and handled by consuming apps
 */
export function createNutritionixClient(): NutritionixClient {
  ...
  if (!appId || !appKey) {
    throw new Error(
      'Missing NUTRITIONIX_APP_ID or NUTRITIONIX_APP_KEY environment variables. ' +
        'Set these in .env.local for development or environment variables for production.'
    );
  }
