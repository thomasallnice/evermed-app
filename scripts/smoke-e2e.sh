...
if echo "$UPLOAD_BODY" | jq . >/dev/null 2>&1; then
  ...
  echo "[upload] non-JSON response: $UPLOAD_BODY"
fi

if [ "$UPLOAD_STATUS" -ne 200 ]; then
  echo "[upload] ERROR: /api/uploads returned HTTP $UPLOAD_STATUS"
  exit 1
fi

DOC_ID=$(echo "$UPLOAD_BODY" | jq -r '.documentId // empty')
[ -z "$DOC_ID" -o "$DOC_ID" = "null" ] && { echo "[upload] missing documentId"; exit 1; }
...
# ---------- Embedding check ----------
echo "[embedding] verifying embeddings exist for $DOC_ID"
...
if [ "$EMBED_COUNT" -eq 0 ]; then
  echo "[embedding] ERROR: no embeddings found for DocChunks"
  exit 1
fi
